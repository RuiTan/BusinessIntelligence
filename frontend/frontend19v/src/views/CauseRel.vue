      ...
      $.ajax({
        ...
        success: function(fb_data) {
          ...
          function buildShow(jsonObj) {
            ...
            var paths_string_div = text_div
              .append("div")
              .attr("class", "paths_string_div");
            ...
            for (var item in data) {
              ...
              for (
                ...
                var node2;
                if (
                  edge.hasOwnProperty("node1") &&
                  edge.hasOwnProperty("node2")
                ) {
                  node1 = edge["node1"];
                  node2 = edge["node2"];
                  ...
                  if (node1Type == 1 && node2Type == 0) {
                    // <--
                    if (
                      !is_included_in_links(my_links, node2Index, node1Index)
                    ) {
                      my_links.push({
                        source: node2Index,
                        target: node1Index,
                    ...
                    // -->
                    if (
                      !is_included_in_links(my_links, node1Index, node2Index)
                    ) {
                      my_links.push({
                        source: node1Index,
                        target: node2Index,
                    ...
                    (node1Type == 2 && node2Type == 2) ||
                    (node1Type == 0 && node2Type == 0)
                  ) {
                    // o--o 两边都是圈的情况, -- 一条线的情况
                    if (
                      !is_included_in_links(my_lines, node1Index, node2Index)
                    ) {
                      my_lines.push({
                        source: node1Index,
                        target: node2Index,
            ...
            function is_included_in_links(_links, _source, _target) {
              for (var i = 0; i < _links.length; i++) {
                ...
                if (
                  cur_link.source === _source &&
                  cur_link.target === _target
                ) {
                  return true;
                }
              }
            ...
            function build(nodes, links, lines) {
              ...
              var svg_nodes = svg
                ...
                .on("click", function(d, i) {
                  if (current_clicked_node_name == d.name) {
                    //点击了两次
              ...
              function recoverBiorientation() {
                ...
                for (let i = 0; i < total_links_lines.length; i++) {
                  if (
                    total_links_lines[i].link_type ===
                      possible_cause_type_line &&
                    total_links_lines[i].is_biorientation === false
                  ) {
              ...
              function hilightPathsAndLines() {
                // sources_path, sources_lines
                svg_links.style("stroke", function(d, i) {
              ...
              function is_cause_path(_source, _target) {
                for (var i = 0; i < sources_path.length; i++) {
                  ...
                  if (
                    _source === cur_path.source.index &&
                    _target === cur_path.target.index
                  ) {
                    return true;
                  }
                }
              ...
              function show_paths_string() {
                build_paths_string();
                d3.select("div.sub_paths_string_div").remove();
                var sub_paths_string_div = d3
                  .select("div.paths_string_div")
                  .append("div")
              ...
              function build_paths_string() {
                ...
                _build_paths_string(
                  target_source_json_obj.children,
                  paths_string[0]
                );
              }
              ...
              function _build_paths_string(children, cur_string) {
                ...
                var cur_string_index_in_paths = paths_string.indexOf(
                  cur_string
                );
...
#submit_btn:hover, #file:hover, #select:hover,svg circle:hover
{
    cursor:pointer;
}
#select
